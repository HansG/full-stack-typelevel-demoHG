...
object Fs2WalRe:
  /*§§
  Stream.eval für setup, dann flatMap für Zielstream
  ...
  /*§§
  ressource aus AutoCloseable
  fs2.Stream aus java.util.stream.Stream
   */
  ...
  /*§§
   Consumer channel.stream - concurrently - 
        Producer Stream.eval: Sync[F].interruptible: impure:JFiles.walkFileTree Visitor:dispatcher.unsafeRunSync(channel.send(...
  impure walkFileTree wird kontrolliert durch: pure 'channel.send ' -> impure Welt per dispatcher.unsafeRunSync
   "sematical blocking channel.send..." per  dispatcher.unsafeRunSync impure und blockiert impure thread JFiles.walk..
  -> Koordination per channel
  ...
  Zusätzliches Einhausen für interruptible:   Sync[F].interruptible: ... - aushausen: mit (Stream.eval) + flatten
  eval(.. Sync[F].interruptible ) er
   */
